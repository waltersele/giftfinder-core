...
function gf_render_final_logic( $atts ) {
    ...
    // PERFILES - ¿Para quién es el regalo?
    $profiles = [
        ['slug'=>'pareja', 'name'=>'Mi pareja', 'icon'=>'fa-heart'],
        ['slug'=>'familia', 'name'=>'Familiar', 'icon'=>'fa-house-user'],
        ['slug'=>'amigo', 'name'=>'Amig@', 'icon'=>'fa-user-group'],
        ['slug'=>'nino', 'name'=>'Niñ@', 'icon'=>'fa-child-reaching'],
        ['slug'=>'yo', 'name'=>'Para mí', 'icon'=>'fa-face-grin-stars']
    ];

    // VIBES - ¿Qué le gusta? (Simplificado a 6 esenciales)
    $vibes = [
        ['slug'=>'tech', 'name'=>'Tech & Gaming', 'icon'=>'fa-gamepad'],
        ['slug'=>'gourmet', 'name'=>'Foodie', 'icon'=>'fa-champagne-glasses'],
        ['slug'=>'bienestar', 'name'=>'Bienestar', 'icon'=>'fa-spa'],
        ['slug'=>'aventura', 'name'=>'Aventura', 'icon'=>'fa-mountain-sun'],
        ['slug'=>'estilo', 'name'=>'Estilo', 'icon'=>'fa-gem'],
        ['slug'=>'fandom', 'name'=>'Fandom', 'icon'=>'fa-wand-magic-sparkles']
    ];
    ...
    if(isset($_GET['mode']) && $_GET['mode'] == 'feed') {
        ...
        // Los tags deben coincidir con las categorías del Hunter: Tech, Gourmet, Zen, Deporte, Viajes, Moda, Friki
        // ============================================================================
        $vibe_mapping = [
            'tech'      => ['Tech'],                  // Tech & Gaming
            'gourmet'   => ['Gourmet'],               // Foodie
            'bienestar' => ['Zen'],                   // Bienestar = Zen (yoga, spa, meditación)
            'aventura'  => ['Viajes', 'Deporte'],     // Aventura = Viajes + Deporte outdoor
            'estilo'    => ['Moda'],                  // Estilo = Moda + Accesorios
            'fandom'    => ['Friki']                  // Fandom = Coleccionismo, Anime, Gaming merch
        ];
        
        // MAPEO DE RECIPIENT
        $recipient_mapping = [
            'pareja'  => ['Mi pareja', 'Amig@'],
            'familia' => ['Familiar', 'Amig@'],
            'amigo'   => ['Amig@', 'Para mí'],
            'nino'    => ['Niñ@'],
            'yo'      => ['Para mí', 'Amig@']
        ];
        
        $vibe_tags = $vibe_mapping[$vibe_slug] ?? ['Friki'];
        $recipient_tags = $recipient_mapping[$rec_slug] ?? ['Amig@'];
        
        // ============================================================================
        // KEYWORDS PARA DETECTAR PRODUCTOS DIGITALES
        // ============================================================================
        $digital_keywords = [
            'ebook', 'kindle', 'digital', 'código', 'codigo', 'descarga', 'suscripción',
            'suscripcion', 'membership', 'online', 'streaming', 'tarjeta regalo', 
            'gift card', 'spotify', 'netflix', 'amazon prime', 'audible', 'curso',
            'masterclass', 'software', 'app', 'licencia', 'key', 'steam', 'playstation store',
            'xbox live', 'nintendo eshop', 'google play', 'itunes', 'apple music'
        ];
        
        // --- FUNCIÓN PARA DETECTAR SI UN TÍTULO ES DIGITAL ---
        function gf_is_digital_product($title) {
            global $digital_keywords;
            $title_lower = strtolower($title);
            foreach($digital_keywords as $kw) {
        ...
        $q = gf_search_with_price([
            'tax_query' => [ 
                'relation' => 'AND',
                ['taxonomy' => 'gf_interest', 'field' => 'name', 'terms' => $vibe_tags, 'operator' => 'IN'],
        ...
        $count_shown = 0;
        $max_results = 20; // Limitar resultados
        
        while($q->have_posts() && $count_shown < $max_results){
            $q->the_post();
            $product_title = get_the_title();
            
            // ============================================================================
            // FILTRO DE TIEMPO/URGENCIA
            // ============================================================================
            if($time_mode == 'immed') {
                // Solo mostrar productos digitales
                if(!gf_is_digital_product($product_title)) {
                    continue; // Saltar productos físicos
                }
            }
        ...
        if($count_shown == 0) {
            $no_results_msg = '🔎 No encontré productos con ese filtro exacto.';
            if($time_mode == 'immed') {
                $no_results_msg = '⚡ No tengo productos digitales en esta categoría todavía. Prueba "24h-48h" para envío rápido.';
            }
            echo '<div class="feed-item" style="background:#0f172a;"><div style="color:white;text-align:center;padding:40px;"><h2 style="font-size:48px;margin-bottom:20px;">🔎</h2><p style="font-size:18px;margin-bottom:30px;">' . esc_html($no_results_msg) . '</p><a href="?" style="color:#818cf8; font-size:16px; text-decoration:underline;">Reiniciar búsqueda</a></div></div>';
        }
        echo '</div>';
