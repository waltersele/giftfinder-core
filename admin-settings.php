 ...
 * - WP_API_TOKEN: Token de autenticación para la API de ingesta
...
function gf_get_env_file_path() {
    // Opciones de búsqueda en orden de preferencia
    $paths = array(
    ...
    // Si ninguno existe, crear en la raíz de WordPress
...
function gf_process_settings_form() {
    ...
    // TAMBIÉN guardar en wp_options como respaldo
    ...
    if ( is_wp_error( $result ) ) {
        ...
        add_action( 'admin_notices', function() {
            echo '<div class="notice notice-success is-dismissible"><p>✅ Variables de entorno guardadas correctamente.</p></div>';
        });
    }
}
...
function gf_render_settings_page() {
    ...
    <div class="wrap gf-admin-wrap">
        <h1>⚙️ Configuración de Giftia</h1>
        ...
        <div style="background: #f5f5f5; border-left: 4px solid <?php echo $env_writable ? '#28a745' : '#dc3545'; ?>; padding: 15px; margin-bottom: 20px; border-radius: 4px;">
            <p><strong>📁 Archivo .env:</strong> <code><?php echo esc_html( $env_file ); ?></code></p>
            <p>
                <strong>Estado:</strong>
                <?php if ( $env_exists ): ?>
                    <span style="color: #28a745;">✅ Existe</span>
                <?php else: ?>
                    <span style="color: #ffc107;">⚠️ No existe (será creado)</span>
                <?php endif; ?>
                
                <?php if ( $env_writable ): ?>
                    <span style="color: #28a745;">✅ Escribible</span>
                <?php else: ?>
                    <span style="color: #dc3545;">❌ No escribible</span>
                <?php endif; ?>
            </p>
            <p style="font-size: 12px; margin-top: 10px;">
                <strong>💡 Tip:</strong> El archivo .env está fuera del directorio public para mayor seguridad.
        ...
        <form method="post" action="" style="max-width: 600px;">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row">
                        <label for="gf_wp_api_token">🔐 Token de API (WP_API_TOKEN)</label>
                    </th>
                    <td>
                        <input 
                            type="password" 
                ...
                <tr>
                    <th scope="row">
                        <label for="gf_gemini_api_key">🤖 Clave API Gemini (GEMINI_API_KEY)</label>
                    </th>
                    <td>
                        <input 
                            type="password" 
                        ...
                        <p class="description">
                            ...
                            <a href="https://aistudio.google.com/app/apikeys" target="_blank">Obtener clave →</a>
                        </p>
                    </td>
                </tr>
                ...
                <tr>
                    <th scope="row">
                        <label for="gf_amazon_tag">🛍️ ID de Afiliado Amazon (AMAZON_TAG)</label>
                    </th>
                    <td>
                        <input 
                            type="text" 
                        ...
                        >
                        <p class="description">
                            Tu ID de afiliado de Amazon. Se añade a todos los enlaces 
                ...
                <tr>
                    <th scope="row">
                        <label for="gf_allowed_origins">🔗 CORS - Orígenes Permitidos (ALLOWED_ORIGINS)</label>
                    </th>
                    <td>
                        <textarea 
                        ...
                        <p class="description">
                            Un origen por línea. Estos dominios pueden acceder a la API.
                            Tu dominio WordPress se incluye automáticamente.
                        </p>
                    </td>
                </tr>
                ...
                <tr>
                    <th scope="row">
                        <label for="gf_debug">🐛 Modo Debug (DEBUG)</label>
                    </th>
                    <td>
                        <input 
                            type="checkbox" 
                        ...
                        <p class="description">Solo usa en desarrollo. Desactiva en producción.</p>
                    </td>
                </tr>
                
            </table>
            
            <?php submit_button( '💾 Guardar Configuración', 'primary', 'submit', true ); ?>
        </form>
        
        <!-- Info Box -->
        <div style="background: #e7f3ff; border: 1px solid #b3d9ff; border-radius: 4px; padding: 15px; margin-top: 30px;">
            <h3>ℹ️ Información de Configuración</h3>
            <ul style="list-style-position: inside;">
                <li><strong>Ubicación .env:</strong> Las variables se guardan en <code><?php echo basename( $env_file ); ?></code></li>
                <li><strong>Fallback:</strong> Si el archivo no existe, usa la base de datos de WordPress</li>
                <li><strong>Seguridad:</strong> El archivo .env NO debe estar en wp-content (está fuera del alcance público)</li>
        ...
        <?php if ( $debug == '1' ): ?>
            <div style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 4px; padding: 15px; margin-top: 20px;">
                <h3>🐛 Variables Cargadas (Debug)</h3>
